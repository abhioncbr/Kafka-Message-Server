...
<licenses>
  ...
  lazy val kafka    = Project(id = "Kafka", base = file(".")).aggregate(core, examples, message, contrib, perf).settings((commonSettings ++
  ...
  lazy val message = Project(id = "java-message-server-example", base = file("message-server-example")).settings(commonSettings :_*) dependsOn (core)
