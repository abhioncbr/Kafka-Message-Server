...
class KafkaProject(info: ProjectInfo) extends ParentProject(info) with IdeaProject {
  ...
  lazy val message  = project("message-server-example", "java-message-server-example", new KafkaMessageExampleProject(_), core)
  ...
  lazy val releaseZip = releaseZipTask dependsOn(core.corePackageAction, core.test, examples.examplesPackageAction, message.messagePackageAction
  ...
  class KafkaMessageExampleProject(info: ProjectInfo) extends DefaultProject(info)
     with IdeaProject
     with CoreDependencies {
    val messagePackageAction = packageAllAction
    val dependsOnCore = core
  //The issue is going from log4j 1.2.14 to 1.2.15, the developers added some features which required
  // some dependencies on various sun and javax packages.
   override def ivyXML =
    ...
    override def artifactID = "kafka-message-server-example"
    override def filterScalaJars = false
    override def javaCompileOptions = super.javaCompileOptions ++
      List(JavaCompileOption("-Xlint:unchecked"))
  }
